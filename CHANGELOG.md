...
- `install-mise-tools`: `tools` parameter (`type: steps`, default `[]`) installs a subset via selector steps such as `revenuecat/mise-java`; an empty list installs all tools from `mise.toml`.
...
- `mise-tool`, `mise-java`, `mise-node`, `mise-yarn`, `mise-ruby`: selector commands for `install-mise-tools.tools`.
