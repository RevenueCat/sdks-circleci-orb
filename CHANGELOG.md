...
- `install-mise-tools`: `tools` parameter installs a subset of tools (e.g. `java` or `node yarn`); when empty, installs all tools from `mise.toml`.
- `install-mise-tools`: `project_root` parameter cd's to the directory containing `mise.toml` before installing.
- `install-mise-tools`: enables mise `experimental` only when `mise.toml` defines `[hooks]` (required for project-level postinstall hooks).
