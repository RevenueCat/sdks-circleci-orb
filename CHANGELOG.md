...
- `install-mise-tools`: include the OS version in the mise cache key. `arch` covers the OS and CPU but not its version, so a toolchain built on one macOS executor could restore onto another running a different macOS. Native extensions bind to the OS, so a Ruby built under macOS 15 (`arm64-darwin24`) restored onto macOS 26 (`arm64-darwin25`) left already-installed gems unusable, failing with "extensions are not built".
